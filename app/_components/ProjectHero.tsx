  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-x-16 lg:gap-y-3 justify-center items-center" >
        {projects.map((project)=> (
          <div key={project.id} className="  col-span-1">
            <Image src={project.image} alt={project.category} width={350} height={350} className="w-[350px] h-[350px] rounded "/>
            <div className=" mt-2 lg:mt-3 ">
                <h4 className="text-[12px] lg:text-sm font-bold text-blue-500 dark:text-white ">{project.category}</h4>
                <p className="text-[12px] lg:text-sm text-gray-500 dark:text-white/80 ">{project.title}</p>
            </div>
        </div>
        ))}
      </div>
